...
public class AlarmSocket {
    ...
    protected static final Logger logger = Logger.getLogger(AlarmSocket.class);
    ...
    public void onWebSocketConnect(Session session) {
        ...
        logger.info("Add to alarm queue: " + session.getId());
        logger.info("Sessions alarm size: " + getSessions().size());
        try {
    ...
    public void onWebSocketText(String message) {
        ...
        if (message.contains("id") && message.contains("area")) {
            Gson gson = new Gson();
    ...
    public void onWebSocketClose(Session session) {
        ...
        logger.info("Socket alarm Closed: " + session);
        logger.info("Remove alarm from queue");
        logger.info("Sessions alarm size: " + getSessions().size());
    }
    ...
    public static boolean checkToSend(String sessionId, String area) {
        String sessionArea = hsmSession.get(md5Id(sessionId));
        if (sessionArea == null) {
            return false;
        }
        return sessionArea.contains("," + area + ",");
    }
